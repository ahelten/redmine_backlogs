...
class RbTask < Issue
  ...
  def self.create_with_relationships(params, user_id, project_id, is_impediment = false)
    ...
    inherited_attribs = inherit_attrs(params)
    if inherited_attribs
      attribs = attribs.merge(inherited_attribs)
    end
